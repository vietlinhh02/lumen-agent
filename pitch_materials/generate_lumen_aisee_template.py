...
with open(matrix_path, 'rb') as f:
    matrix_b64 = "data:image/png;base64," + base64.b64encode(f.read()).decode('utf-8')

with open(gaps_path, 'rb') as f:
    gaps_b64 = "data:image/png;base64," + base64.b64encode(f.read()).decode('utf-8')
...
<html lang="vi">
  <head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&family=Plus+Jakarta+Sans:wght@500;600;700;800&family=JetBrains+Mono:wght@400;500;600&display=swap" rel="stylesheet">
    <style>
      :root {{
        --brand: #002043;
        --ink: #171a1b;
      ...
      * {{
      ...
      .cover {{
        background: linear-gradient(165deg, rgba(232, 238, 244, 0.92), rgba(255, 255, 255, 0.98));
      }}
      ...
      .cover .hero-img {{
        ...
        border-radius: 8px;
        border: 1px solid var(--line);
        box-shadow: 0 10px 30px rgba(0, 32, 67, 0.08);
      }}
  ...
  <body>
    <main class="deck">
      ...
      <section class="slide cover">
        <img class="hero-img" src="{matrix_b64}" alt="Lumen Comparison Matrix Interface" />
        <div class="top-row">
          <span class="pill">Báo cáo Kỹ thuật · AI Agent System</span>
          <div class="logo">lumen</div>
        </div>

        <div style="max-width: 6.8in;">
          <h1 style="margin-top: 0.42in">Lumen - Trợ lý AI Agent Tự hành Nghiên cứu Khoa học Đa tài liệu</h1>
          <p class="tagline">Tự động hóa tổng quan tài liệu học thuật với độ chính xác và tính minh bạch tuyệt đối</p>
          <p class="lead" style="margin-top: 0.12in;">
            Hệ sinh thái Multi-Agent được phát triển chuyên sâu nhằm giải quyết triệt để rủi ro tạo ảo giác (hallucination), tự động hóa việc đọc hiểu, trích xuất cấu trúc dữ liệu PDF phức tạp và lập ma trận so sánh đối chiếu đa tài liệu với cơ chế kiểm chứng nguồn cấp dòng.
          </p>
          <div class="metrics" style="margin-top: 0.34in; grid-template-columns: repeat(2, 1fr); gap: 0.14in;">
            <div class="metric"><strong>100%</strong><span>Nhận định đối chiếu chính xác đến dòng và trang PDF</span></div>
            <div class="metric"><strong>3.4x</strong><span>Cải thiện trích xuất bảng biểu phức tạp với Marker API</span></div>
            <div class="metric"><strong>+28%</strong><span>Tăng độ bao phủ truy xuất (Recall@10) nhờ Hybrid Search</span></div>
            <div class="metric"><strong>-80%</strong><span>Tiết kiệm chi phí token nhờ luồng Routing & Clean Data</span></div>
          </div>
        </div>
      ...
      <section class="slide">
        ...
        <h2 style="margin-top: 0.16in">Rào cản Nghiên cứu & Rủi ro Ảo giác từ Mô hình Ngôn ngữ</h2>
        <p class="lead">Các mô hình ngôn ngữ tổng quát tiềm ẩn nguy cơ sai lệch nghiêm trọng khi xử lý dữ liệu học thuật, trong khi quy trình tổng quan tài liệu thủ công lại quá tốn thời gian và dễ bỏ sót thông tin cốt lõi.</p>
        ...
        <div class="grid-2" style="margin-top: 0.2in">
          ...
          <article class="card primary">
            ...
            <ul>
              ...
              <li>Thiết lập cơ chế kiểm chứng nghiêm ngặt đảm bảo 100% khẳng định có Chunk ID định vị trong PDF.</li>
      ...
      <section class="slide">
        ...
        <h2 style="margin-top: 0.16in">Hệ thống 3 Tầng Bảo vệ Loại bỏ Hoàn toàn Ảo giác</h2>
        <p class="lead">Thiết lập hàng rào kiểm soát chất lượng đa tầng từ bước chọn lọc tài liệu đầu vào, gán nhãn độ tin cậy trung gian đến phê duyệt báo cáo đầu ra.</p>
        ...
        <div class="grid-3" style="margin-top: 0.22in">
          ...
          <article class="card">
            ...
            <h3>TẦNG 3: XÁC THỰC TUYỆT ĐỐI</h3>
            <p>Báo cáo tổng hợp cuối cùng chỉ được phê duyệt xuất bản khi 100% các nhận định đều được liên kết chính xác với Chunk ID đã được lưu trữ trong cơ sở dữ liệu. Mọi câu khẳng định thiếu nguồn đối chiếu đều bị loại bỏ ngay lập tức.</p>
          </article>
        </div>

        <div class="strip" style="margin-top: 0.2in">
          <strong>Nguyên tắc Bất biến</strong>
          <p>Không bao giờ cho phép LLM suy đoán hoặc bổ sung kiến thức nằm ngoài tập dữ liệu tài liệu đã truy xuất. Mọi luận điểm khoa học bắt buộc phải truy vết được đến vị trí vật lý trong văn bản gốc.</p>
        </div>
        ...
        <div class="metrics" style="margin-top: 0.2in; grid-template-columns: repeat(3, 1fr);">
          ...
          <div class="metric"><strong>100% Verified</strong><span>Tỷ lệ nhận định bắt buộc liên kết Chunk ID</span></div>
        </div>
      ...
      <section class="slide">
        ...
        <div style="display: grid; grid-template-columns: 5.6in 1fr; gap: 0.22in; margin-top: 0.24in; align-items: stretch;">
          <div style="display: flex; flex-direction: column; gap: 0.16in;">
            ...
            <article class="card primary" style="flex: 1;">
          ...
          <div style="display: flex; flex-direction: column; justify-content: space-between;">
            <div style="border: 1px solid var(--line); border-radius: 8px; overflow: hidden; background: #fff; box-shadow: 0 8px 24px rgba(0,32,67,0.06); height: 3.1in;">
            ...
            <div style="border: 1px solid var(--line); border-radius: 8px; background: var(--soft); padding: 0.1in 0.16in; display: flex; align-items: center; justify-content: space-between;">
              <div>
                <strong style="color: var(--brand); font-size: 11px; display: block;">Phát hiện Mâu thuẫn & Khoảng trống Nghiên cứu (Gaps & Conflicts)</strong>
                <span style="color: var(--muted); font-size: 9.5px;">Tự động tổng hợp các thiếu sót thực nghiệm và mâu thuẫn giữa các công trình khoa học</span>
              </div>
              <span class="pill" style="min-height: 0.22in; font-size: 8px;">Gaps Identified</span>
            </div>
          </div>
        </div>
      ...
      <section class="slide">
          ...
          <table>
            <thead>
              <tr>
                ...
                <th style="width: 24%; color: var(--brand); background: var(--badge);">Lumen Multi-Agent (Đề xuất)</th>
              </tr>
            </thead>
            <tbody>
              <tr>
                <td>Tỷ lệ Ảo giác Trích dẫn (Hallucination)</td>
                <td>47.0% (Rất cao)</td>
                <td>18.5% (Còn sai sót)</td>
        ...
        <div class="metrics" style="margin-top: 0.2in; grid-template-columns: repeat(4, 1fr);">
          ...
          <div class="metric"><strong>-80%</strong><span>Chi phí API nhờ Routing</span></div>
          <div class="metric"><strong>96.4%</strong><span>Độ bao phủ truy xuất Recall@10</span></div>
          <div class="metric"><strong>45 Giây</strong><span>Thời gian xử lý ma trận 10 bài</span></div>
        </div>
      ...
      <section class="slide thanks">
        <div style="max-width: 6.8in;">
          <h1 style="margin-top: 0.42in; color: #fff; font-size: 38px;">TỔNG KẾT & TRAO ĐỔI KỸ THUẬT</h1>
          ...
          <p style="margin-top: 0.14in; font-size: 11.2px; color: rgba(255,255,255,0.72); line-height: 1.58;">
            Hệ thống giải quyết trọn vẹn bài toán trích xuất dữ liệu học thuật phức tạp, kết hợp luồng Multi-Agent kiểm soát trạng thái chặt chẽ và hệ thống Guardrails 3 tầng loại bỏ hoàn toàn hiện tượng tạo ảo giác. Sẵn sàng cho triển khai và mở rộng quy mô.
          </p>
          ...
          <div style="margin-top: 0.3in; display: flex; flex-direction: column; gap: 0.12in;">
            <div style="display: flex; align-items: center; gap: 0.14in; background: rgba(255,255,255,0.08); padding: 0.12in 0.18in; border-radius: 8px; border: 1px solid rgba(255,255,255,0.15);">
              <strong style="color: #fff; font-size: 12px; min-width: 1.6in;">Độ tin cậy tuyệt đối:</strong>
              <span style="color: rgba(255,255,255,0.85); font-size: 10.8px;">100% nhận định đối chiếu đến từng số trang và dòng văn bản PDF</span>
            </div>
            <div style="display: flex; align-items: center; gap: 0.14in; background: rgba(255,255,255,0.08); padding: 0.12in 0.18in; border-radius: 8px; border: 1px solid rgba(255,255,255,0.15);">
              <strong style="color: #fff; font-size: 12px; min-width: 1.6in;">Tối ưu hóa hiệu năng:</strong>
            ...
            <div style="display: flex; align-items: center; gap: 0.14in; background: rgba(255,255,255,0.08); padding: 0.12in 0.18in; border-radius: 8px; border: 1px solid rgba(255,255,255,0.15);">
              <strong style="color: #fff; font-size: 12px; min-width: 1.6in;">Trải nghiệm minh bạch:</strong>
        ...
        <div class="thanks-card">
          ...
          <p style="color: var(--muted); font-size: 10px; margin-top: 0.04in;">Mã nguồn mở và tài liệu kiểm thử hệ thống</p>

          <div style="margin-top: 0.2in; padding: 0.12in 0.15in; border-radius: 8px; background: var(--soft); border: 1px solid var(--line);">
            <span style="display: block; font-size: 9px; font-weight: 700; text-transform: uppercase; color: var(--muted); letter-spacing: 0.5px;">Kho mã nguồn (GitHub Repository)</span>
            <strong style="display: block; font-size: 13px; color: var(--brand); margin-top: 2px;">github.com/linhnv-hub/lumen</strong>
            <span style="display: block; font-size: 9.6px; color: var(--muted); margin-top: 2px;">Toàn bộ source code Backend (FastAPI), Frontend (React) và LangGraph Workflows</span>
          </div>

          <div style="margin-top: 0.12in; padding: 0.12in 0.15in; border-radius: 8px; background: var(--soft); border: 1px solid var(--line);">
            <span style="display: block; font-size: 9px; font-weight: 700; text-transform: uppercase; color: var(--muted); letter-spacing: 0.5px;">Video Minh họa Tác tử (Demo Screencast)</span>
            <strong style="display: block; font-size: 13px; color: var(--brand); margin-top: 2px;">youtu.be/lumen-agent-demo</strong>
            <span style="display: block; font-size: 9.6px; color: var(--muted); margin-top: 2px;">Quá trình Agent tự hành tìm kiếm, trích xuất và lập ma trận đối chiếu thời gian thực</span>
          </div>

          <div style="margin-top: 0.12in; padding: 0.12in 0.15in; border-radius: 8px; background: var(--soft); border: 1px solid var(--line);">
            <span style="display: block; font-size: 9px; font-weight: 700; text-transform: uppercase; color: var(--muted); letter-spacing: 0.5px;">Tài liệu Kiến trúc & API Specs</span>
            <strong style="display: block; font-size: 13px; color: var(--brand); margin-top: 2px;">docs.lumen-agent.dev/architecture</strong>
            <span style="display: block; font-size: 9.6px; color: var(--muted); margin-top: 2px;">Sơ đồ StateGraph, đặc tả schemas Pydantic và cấu hình Qdrant Vector Index</span>
          </div>

          <p class="quote">
            "Trong nghiên cứu học thuật, một kết quả tổng quan chỉ thực sự có giá trị khi mọi khẳng định đều được kiểm chứng độc lập tới từng dòng trong văn bản gốc."
          </p>
        </div>

        <div class="logo" style="position: absolute; left: 0.72in; bottom: 0.3in; color: #fff;">lumen</div>
        <p style="position: absolute; right: 0.72in; bottom: 0.3in; font-size: 9px; color: rgba(255, 255, 255, 0.55);">
...
print(f"Generated finalized HTML at {output_path} ({len(html_content)} bytes)")
