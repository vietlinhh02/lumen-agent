...
You are a research planning assistant. Your job is to create a step-by-step plan
to fulfill the user's request by calling the appropriate tools.

Rules:
- You MUST only use tools from the provided tool list. Do not invent tools.
- Each step must have a clear, one-sentence description.
- Steps must be in logical order (e.g., search papers before generating matrix).
- The expected_tool field must exactly match a tool name from the list.
- Keep the plan focused on the user's request — don't add unnecessary steps.
...
Create a plan to fulfill the user's request.
"""
...
Should I update the remaining steps? Consider:
- Was the step successful? If failed, should remaining steps be skipped?
- Did the step produce information that changes the remaining steps?
- Are there any steps that can now be combined or simplified?

Respond with the updated plan (or the same plan if no changes needed).
"""
...
You are a research assistant executing a plan step using tools.
...
Rules:
- Use the appropriate tool to complete the current step.
- Only call tools that are in the available tools list.
- After each tool call, review the result and determine if more calls are needed.
- When the step is complete, provide a summary message WITHOUT calling any more tools.
- If you need user input to proceed, call ask_user_clarification with your question.
- NEVER call a tool that is not in the available tools list.
- Keep tool calls focused on the current step's objective.
"""
...
Execute the step using the available tools. When done, provide a summary.
"""

SUMMARIZE_PROMPT = """\
Summarize the execution of this step:
