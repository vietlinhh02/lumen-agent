...
export default function DashboardPage() {
  ...
  const quickActions = [
    {
      label: "Search Papers",
      description: "Find papers from academic databases",
    ...
    {
      label: "View Matrix",
      description: "Compare papers side by side",
      icon: Table,
      href: "/matrix",
      color: "text-emerald-600",
      bg: "bg-emerald-50",
    },
    {
      ...
      description: "Create a citation-safe literature review",
      icon: PencilLine,
      href: "/reports",
      color: "text-violet-600",
      bg: "bg-violet-50",
    ...
    <div className="animate-fade-in">
      ...
      <div className="mb-8">
        ...
        <p className="mt-2 text-base text-charcoal">
          Your AI literature review workspace at a glance.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 mb-8">
        {stats
          ? statCards.map((card) => (
              <div
                key={card.label}
                className="rounded-[12px] bg-surface-card p-4 transition-shadow hover:shadow-sm"
                style={{ border: "1px solid var(--hairline)" }}
              >
                <div className="flex items-center gap-3">
                  <div
                    className={`flex h-[36px] w-[36px] items-center justify-center rounded-[10px] ${card.bg}`}
                  >
                    <card.icon size={18} className={card.color} />
                  </div>
                  <div>
                    <p className="font-display text-[24px] font-bold text-ink leading-none">
                      {card.value}
                    </p>
                    <p className="font-ui text-[11px] text-ash mt-0.5">
                      {card.label}
                    </p>
                  </div>
                </div>
          ...
          : [1, 2, 3, 4, 5].map((i) => (
              <div
                key={i}
                className="rounded-[12px] bg-surface-card p-4 animate-pulse"
                style={{ border: "1px solid var(--hairline)" }}
              >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1">
          <h2 className="font-ui text-[12px] font-semibold text-ash uppercase tracking-wide mb-3">
            Quick Actions
          </h2>
          <div className="space-y-2">
            {quickActions.map((action) => (
              ...
              >
                <div
                  className={`flex h-[32px] w-[32px] items-center justify-center rounded-[8px] ${action.bg} shrink-0`}
                >
                ...
                <ArrowRight
                  size={14}
                  className="text-stone group-hover:text-ink transition-colors shrink-0"
                />
              </button>
            ))}
          </div>
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <p className="font-ui text-[14px] font-medium text-ink truncate">
                          {p.title}
                        </p>
                        <p className="font-ui text-[11px] text-ash">
