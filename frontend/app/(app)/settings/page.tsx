...
function ProfileSection({
  ...
  return (
    <div className="space-y-6">
      {/* Account Info */}
      <div
        className="rounded-[12px] bg-surface-card p-6"
        style={{ border: "1px solid var(--hairline)" }}
      >
        <h2 className="font-ui text-base font-semibold text-ink mb-4">
          Account Information
        </h2>
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="font-ui text-sm font-medium text-ink capitalize">
              {profile.role}
            </span>
          </div>
          <div className="flex items-center justify-between">
            ...
            <span
              className={`font-ui inline-flex items-center gap-1 text-sm font-medium ${
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              ...
              <p className="font-ui text-[12px] text-charcoal">
          ...
          <div
            className={`relative h-[28px] w-[48px] rounded-full transition-colors ${
              theme === "dark" ? "bg-primary" : "bg-stone/30"
            }`}
          >
            <div
      ...
      <div
        ...
        style={{ border: "1px solid var(--hairline)" }}
      >
        <h2 className="font-ui text-base font-semibold text-ink mb-4 flex items-center gap-2">
          <Lock size={18} />
          Change Password
        </h2>
        <form onSubmit={handleChangePassword} className="space-y-4 max-w-sm">
          ...
          <button
            type="submit"
            disabled={saving}
            className="focus-ring font-ui h-[44px] rounded-full bg-primary px-5 text-sm font-semibold text-on-primary transition-colors hover:bg-primary-deep disabled:opacity-50"
          >
    ...
    >
      ...
      {loading ? (
        <div className="p-6 space-y-3">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-12 rounded bg-surface-bone animate-pulse" />
          ))}
        </div>
      ) : (
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-surface-bone/50">
                <th className="font-ui text-[11px] font-semibold text-ash uppercase tracking-wide text-left px-6 py-3">
                  Email
                </th>
                <th className="font-ui text-[11px] font-semibold text-ash uppercase tracking-wide text-left px-6 py-3">
                  Role
                ...
                <th className="font-ui text-[11px] font-semibold text-ash uppercase tracking-wide text-right px-6 py-3">
                  Actions
                </th>
              </tr>
            </thead>
            <tbody>
              {users.map((u) => (
                ...
                >
                  <td className="px-6 py-3 font-ui text-sm text-ink">
                    {u.email}
                  </td>
                  <td className="px-6 py-3 font-ui text-sm text-charcoal capitalize">
                    {u.role}
                  </td>
                  <td className="px-6 py-3">
                    <span
                      className={`font-ui inline-flex items-center gap-1 text-[12px] font-semibold ${
                  ...
                  <td className="px-6 py-3 text-right">
                    <button
                      ...
                      className={`font-ui text-[12px] font-semibold px-3 py-1.5 rounded-full transition-colors ${
                        u.is_active
