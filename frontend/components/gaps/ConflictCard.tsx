...
export function ConflictCard({ conflict, expanded, onToggle, confidenceColor }: Props) {
  return (
    <div
        ...
        <div className="px-5 pb-5 space-y-4 border-t border-[var(--hairline)] pt-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="rounded-[8px] bg-green-50 px-4 py-3">
              <p className="font-ui text-[12px] font-semibold text-green-700 mb-1">{conflict.paper_a_title || "Paper A"}</p>
              <p className="text-sm text-ink">{conflict.claim_a || "No claim specified"}</p>
            </div>
            <div className="rounded-[8px] bg-red-50 px-4 py-3">
              <p className="font-ui text-[12px] font-semibold text-red-700 mb-1">{conflict.paper_b_title || "Paper B"}</p>
