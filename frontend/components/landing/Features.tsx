    ...
    const ctx = gsap.context(() => {
      ...
      if (heading) {
        ...
        if (h2) {
          ...
          h2.innerHTML = text
            .split(" ")
            .map((w) => `<span class="word" style="perspective:400px">${w}&nbsp;</span>`)
            .join("");
        }
      }
    ...
    <section ref={sectionRef} id="features" className="py-24 lg:py-32">
      <div className="mx-auto max-w-7xl px-6">
          ...
          {features.map((feature, i) => (
            <div key={i} className="feature-card group overflow-hidden rounded-xl border border-hairline bg-surface-card transition-all hover:border-hairline-strong hover:shadow-lg">
              ...
              <div className="p-6">
                ...
                <p className="mb-4 text-sm leading-relaxed text-charcoal">{feature.description}</p>
                <div className="flex flex-wrap gap-2">
                  {feature.tags.map((tag) => (
                    <span key={tag} className="feature-tag rounded-full border border-hairline bg-canvas px-3 py-1 text-xs font-medium text-charcoal">{tag}</span>
                  ))}
                </div>
              </div>
            </div>
        ...
        <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
          {[
            { title: "Knowledge Map", desc: "Interactive graph of papers, methods, datasets." },
            { title: "Research Gaps", desc: "Evidence-backed gap detection with paper references." },
            { title: "Conflict Detection", desc: "Flag opposing findings across studies." },
